...
public class PhoneBook {
	...
	private void insertPerson(Person newPerson, Person currentNode){
		if(compareTwoPersons(newPerson, currentNode) < 0) {
			if(currentNode.getLeftSuccessor() == null) currentNode.setLeftSuccessor(newPerson);
			else insertPerson(newPerson, currentNode.getLeftSuccessor());
		} else {
			if(currentNode.getRightSuccessor() == null) currentNode.setRightSuccessor(newPerson);
			else insertPerson(newPerson, currentNode.getRightSuccessor());
		}
	}
	...
	private boolean findPerson(String lastName, String firstName, int number, Person currentNode){
		...
		int temp = compareTwoPersons(lastName, firstName, number, currentNode);
		if(temp == 0) {
			return true;
		} else if(temp < 0) {
