...
public class Aufgabe2 {
	...
	public static int convertColorToGrayscale(int rgb) {
		int zahl = (Colors.getR(rgb)+Colors.getG(rgb)+Colors.getB(rgb))/3;
		return Color.HSBtoRGB(zahl, zahl, zahl);
	}
        ...
        for (int x = 0; x < pixels.length; x++) {
            for (int y = 0; y < pixels[x].length; y++) {
                if (x < pixels.length / 3) {
                    pixels[x][y] = Colors.toRGB(Colors.getR(pixels[x][y]), 0, 0);
                } else if (x >= pixels.length / 3 && x < pixels.length * 2 / 3) {
                    pixels[x][y] = Colors.toRGB(0, Colors.getG(pixels[x][y]), 0);
                } else {
                    pixels[x][y] = Colors.toRGB(0, 0, Colors.getB(pixels[x][y]));
                }
            }
        }
